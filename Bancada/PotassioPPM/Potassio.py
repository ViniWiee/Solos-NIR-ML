...
wl = np.asarray(list(Dados.columns.values)[2:553],dtype = "int")
...
def find_Best_Spectrum(X,y):
    ...
    return (optimalSpectrum,optimalPLSComponents,o,n,w,s)

selectedSpectrum, optimalPLSComponents, opt_Xc, ncomp, wav, sorted_ind = find_Best_Spectrum(savitskyGolaySpectrum,y)

ix = np.in1d(wl, wl[sorted_ind][:wav])
...
with plt.style.context(('ggplot')):
    ax.plot(wl, savitskyGolaySpectrum.T)
...
def kennardStonePLS(X_train, y_train,X_test, y_test, n_components):
    ...
    rmse_calib, mae_p, score_calib = np.sqrt(mean_squared_error(y_train, y_calib)), mean_absolute_error(y_train, y_calib), r2_score(y_train, y_calib)
...
def findBestContamination(X, y):
    ...
    for i in range(30):
        ...
        if( newScore > score and overfit < 0.02):
            score = newScore
...
def printKennardStonePLS(X_train, y_train,X_test, y_test, n_components):
    ...
    # metrics in prediction
    ## Calibration Metrics
    rmse_calib, mae_calib, score_calib, sdc = np.sqrt(mean_squared_error(y_train, y_calib)), mean_absolute_error(y_train,                                                                                  y_calib), r2_score(
        y_train, y_calib) , np.std(y_train)
    rpdc = sdc / rmse_calib
    ## Validation Metrics
