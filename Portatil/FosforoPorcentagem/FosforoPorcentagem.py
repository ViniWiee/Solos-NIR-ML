...
y = np.asarray(Dados.iloc[:,263])*1000
...
def printKennardStonePLS(X_train, y_train,X_test, y_test, n_components):
    ...
    # test metrics
    rmse_p, mae_p, score_p ,sd = np.sqrt(mean_squared_error(y_test, y_pred)), mean_absolute_error(y_test, y_pred), r2_score(y_test, y_pred), np.std(y_test)
    rpd = sd / rmse_p
    #calibration
    ...
    with plt.style.context(('ggplot')):
        ...
        plt.xlabel('Fosforo Medido (porcentagem)')
        plt.ylabel('Fosforo Predito (porcentagem)')
